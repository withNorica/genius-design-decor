  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultToStore: GenerationResult = {
        ...
        style: submissionStyle, details, 
        suggestions: {
          general: resultData.suggestions.split('\n').filter((s: string) => s.startsWith('•')).map((s: string) => s.substring(1).trim()),
          lowBudget: [],
          diy: [],
        },
...
const parseSuggestions = (text: string | DesignSuggestions | DecorSuggestions) => {
    if (typeof text !== 'string') {
        return text; // It's already an object
    }
    const general = text.match(/Actionable Suggestions:([\s\S]*?)Budget-Friendly:/)?.[1]?.trim().split('\n').filter(Boolean) || [];
    const lowBudget = text.match(/Budget-Friendly:([\s\S]*?)DIY Ideas:/)?.[1]?.trim().split('\n').filter(Boolean) || [];
    const diy = text.match(/DIY Ideas:([\s\S]*)/)?.[1]?.trim().split('\n').filter(Boolean) || [];
    if (general.length === 0 && lowBudget.length === 0 && diy.length === 0 && text) {
      return { general: text.split('\n').filter(Boolean), lowBudget: [], diy: [] };
    }
    return { general, lowBudget, diy };
};

const SuggestionsList: React.FC<{title: string; items: string[]}> = ({ title, items }) => {
    if (!items || items.length === 0) return null;
    return ( <div className="mb-6"><h3 className="text-lg font-bold mb-3 text-[#E75480]">{title}:</h3><ul className="list-disc list-inside space-y-2 text-gray-700">{items.map((item, index) => <li key={index}>{item}</li>)}</ul></div> );
}

const DesignSuggestionsDisplay: React.FC<{suggestions: DesignSuggestions | string}> = ({ suggestions }) => {
    const parsed = parseSuggestions(suggestions);
    return ( <>{parsed.general && <SuggestionsList title="Actionable Suggestions" items={parsed.general} />}{parsed.lowBudget && <SuggestionsList title="Budget-Friendly" items={parsed.lowBudget} />}{parsed.diy && <SuggestionsList title="DIY Ideas" items={parsed.diy} />}</> );
};

const DecorSuggestionsDisplay: React.FC<{suggestions: DecorSuggestions | string}> = ({ suggestions }) => {
    const parsed = parseSuggestions(suggestions);
    return ( <>{parsed.general && <SuggestionsList title="Actionable Suggestions" items={parsed.general} />}{parsed.lowBudget && <SuggestionsList title="Budget-Friendly" items={parsed.lowBudget} />}{parsed.diy && <SuggestionsList title="DIY Ideas" items={parsed.diy} />}</> );
};
